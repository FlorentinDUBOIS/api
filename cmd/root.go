...
func init() {
	...
	RootCmd.Flags().String("jwt-secret", "", "Set the JWT application secret")
	RootCmd.Flags().String("jwt-realm", "localhost", "Set the JWT application realm")
...
func bouncer(cmd *cobra.Command, args []string) {
	...
	authController.Register(api.Group("/auth"))
